...
def evaluate_single_net(
    ...
    """Evaluation, but with only 1 Q network as opposed to using multiple parameters."""
    ...
    # note: critic_params is not used in this script
    with open(model_path, "rb") as f:
        (args, (network_params,)) = flax.serialization.from_bytes(
            (None, (network_params,)), f.read()
        )
    ...
    def get_action_and_value(
        ...
        logits = network.apply(network_params, next_obs)
        # sample action: Gumbel-softmax trick
        # see https://stats.stackexchange.com/questions/359442/sampling-from-a-categorical-distribution
        key, subkey = jax.random.split(key)
        u = jax.random.uniform(subkey, shape=logits.shape)
